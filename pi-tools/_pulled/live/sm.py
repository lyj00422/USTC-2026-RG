...
class RouteState(str, Enum):
    ...
    # rather than copied.
    JUNCTION_2_BACK_TO_LINE = "JUNCTION_2_BACK_TO_LINE"
    ...
    #
    # J3 is where the run stops being a single line.  Three legs leave it, each
    # ending in a wall contact at a pickup/build area, and the first two return
    # to J3 and turn again.  The arm will be attached at the *_ARRIVED states.
    #
    #   trip 1  left 90   seek RIGHT  ->  PICKUP_ARRIVED     (area 2)
    #   trip 2  right 90  seek LEFT   ->  PICKUP_2_ARRIVED   (area 1)
    #   trip 3  right 180 no seek, already on the line        (build area)
    #
    ...
    # and the speed selection at the end of step() keys on the name.  Since
    # 2026-09-15 that selection reads follows_line() instead, but keeping the
    # prefix costs nothing and the old rule is one refactor away from returning.
...
class RouteV2StateMachine:
    ...
    def __init__(self, config: RouteV2Config):
        ...
        self._j2_recovery = LineRecoveryController(
            segment_cm=config.j2_line_recovery_segment_cm
        )
        self._j2_recovery_origin_cm: float | None = None
        self._j2_recovery_distance_cm = 0.0
        self._j2_recovery_failed = False
        self._j2_recovery_settled = False
    ...
    def _enter(self, state: RouteState, now: float) -> None:
        ...
        if state is RouteState.PURPLE_PRESCAN:
            # A new round starts only on arrival at J3.  This preserves the
            # monotonic purple_mode flag while clearing the previous round's
            # collected counts and orange action index.
            self.loop_context.start_round()
        if state is RouteState.JUNCTION_2_BACK_TO_LINE:
            self._j2_recovery = LineRecoveryController(
                segment_cm=self.config.j2_line_recovery_segment_cm
            )
            self._j2_recovery_origin_cm = None
            self._j2_recovery_distance_cm = 0.0
            self._j2_recovery_failed = False
            self._j2_recovery_settled = False
    ...
    def _strafe_line_seek(self, now: float, sensor_mask: int, lateral_cm: float | None,
        ...
        if sensor_mask in self.config.seek_line_masks:
            return None
        if accept_any_black:
            # The J2 rule (see _j2_recover_line): any black short of line loss.
            # The all-black reading counts on purpose -- on the way back to J3
            # it IS the landmark, and the old PICKUP_1_RETURN rule was
            # "mask == 0x00".
        ...
        if now - self._strafe_seek_started >= timeout_s:
            return RouteIntent("stop", self.state)
        reach = self.config.j2_line_recovery_segment_cm if segment_cm is None else segment_cm
        if self._strafe_seek_origin_cm is None:
            ...
            if not self._strafe_seek_settled:
                # lateral_cm is re-based on the tick AFTER a state change, so this
                # state's first sample still belongs to the previous leg.  Take
                # the origin from the next, settled one -- the same defect that
                # latched _j2_recovery_failed dead on 2026-09-22.
    ...
    def _j2_recover_line(self, now: float, sensor_mask: int, travel_cm: float | None,
                         lateral_cm: float | None = None) -> RouteIntent | None:
        """Hand straight over to the left turn -- the hunt happens AFTER it.

        Operator, 2026-09-22: "我的意思是 先左旋90 再找线".  At the J2 L-corner the
        line is lost, the car turns LEFT 90 degrees, and only THEN hunts for the
        line.  That hunt is JUNCTION_3_SEEK_LINE, which alternates left/right in
        ten-centimetre steps.

        So this state does not hunt at all any more.  Hunting here seeks a line
        the car is about to turn away from -- and the longitudinal hunt this
        replaced (reverse/forward in 5 cm steps) was worse still: reversing at
        an L-corner moves the car away from the line it lost.

        Returning None on the first tick makes the caller fall straight through
        to `_enter(JUNCTION_2_TURN_LEFT)` on that same tick, the way the J1
        alignment falls through once it is aligned.

        `_j2_recovery_*`, `LineRecoveryController` and the
        `junction_2_back_*` bounds are now unused by this state.
        """
        return None
    ...
    def _creep_back_onto(self, now: float, sensor_mask: int, travel_cm: float | None, *,
        ...
        probe on black at all.  It exists for the J2 reverse, where the bar
        arrives on the line from an unknown lateral offset and therefore reads
        an arbitrary slice of it -- measured 2026-09-17, see the field decision
        on junction_2_back_accept_any_black in config.py.  0xFF is excluded
        structurally rather than by choice: it is line loss, and line loss is
        what put the car in this state, so accepting it would fire on tick one.

        forward_cm adds a second phase, and is 0 at J2.  The reverse phase rests
    ...
    def step(self, now: float, *, sensor_mask: int = 0, line_error: float | None = 0.0,
        ...
        if self.state is RouteState.JUNCTION_2_LINE_FOLLOW:
            ...
            # rather than straight to the turn -- see JUNCTION_2_BACK_TO_LINE.
            if self._line_lost_and_travelled(
                    sensor_mask, travel_cm,
                    gate_cm=self.config.junction_2_line_loss_gate_cm,
                    ceiling_cm=self.config.junction_2_max_travel_cm):
                self._enter(RouteState.JUNCTION_2_BACK_TO_LINE, now)
            # Falls through to the line-following return at the end of step().

        if self.state is RouteState.JUNCTION_2_BACK_TO_LINE:
            backing = self._j2_recover_line(now, sensor_mask, travel_cm,
        ...
        if self.state is RouteState.PURPLE_PRESCAN:
            if not self.loop_context.purple_mode:
                # Purple mode is monotonic.  Once disabled, J3 is only a
                # transit point and the next leg is the orange area.
                self._enter(RouteState.PICKUP_2_TURN_RIGHT, now)
            elif visual.purple_prescan_present is None:
                ...
                if self.purple_target_slot is None:
                    self.loop_context.disable_purple("j3_no_purple")
                    self._enter(RouteState.PICKUP_2_TURN_RIGHT, now)
                else:
        ...
        if self.state is RouteState.PICKUP_VISION_ONLY:
            if visual.pickup_kind == "fault":
                ...
                self.loop_context.disable_purple("purple_grab_failed")
            ...
            if visual.action_done:
                ...
                self.loop_context.disable_purple("purple_target_gone")
        ...
        if self.state is RouteState.PICKUP_2_SEEK_LINE:
            ...
            #
            # The gate is the DISABLE REASON, not `purple_mode` alone: a round
            # whose purple grab failed also leaves purple_mode off, but that car
            # did drive to the area, so it keeps the configured direction.
            vy = self.config.pickup_2_seek_line_vy
            if (not self.loop_context.purple_mode
                    and self.loop_context.purple_disable_reason == "j3_no_purple"):
        ...
        if self.state is RouteState.PICKUP_2_VISION_ONLY:
            ...
            if visual.action_done:
                ...
                self.loop_context.orange_action_index += 1
                if self.loop_context.orange_action_index < len(self.loop_context.orange_actions):
                    # Keep the car in area 1.  The runtime resets its bounded
                    # lateral search and arms the next placeholder action.
                    self._enter(RouteState.PICKUP_2_VISION_ONLY, now)
                    return RouteIntent("stop", self.state)
                self._enter(RouteState.PICKUP_2_RETURN_TO_LINE, now)
            elif visual.pickup_kind == "no_target":
                # The round is over; record what it collected and carry it to the
                # build area.  Every completed round returns to J3.
                record_round_result(self.loop_context)
                self._enter(RouteState.PICKUP_2_RETURN_TO_LINE, now)
            elif visual.pickup_kind in {"strafe_right", "strafe_left", "return_baseline",
                                        "align_left", "align_right"}:
                return RouteIntent("strafe", self.state, speed=visual.pickup_speed)
        ...
        if self.state is RouteState.JUNCTION_PICKUP_3_TO_AREA:
            ...
            #
            # ARRIVAL IS THE COLLISION, and nothing else.  A distance gate was
            # tried here on 2026-09-15 and withdrawn the same night: the operator
            # put the leg at "j2到搭建区大概是60cm", the car duly stopped at
            # 62.41 cm, and the leg is really about **2.6 m** -- so the gate
            # parked it roughly two metres short of the build area.  The tape
            # measurement was wrong, not the criterion; this leg is long and its
            # end is the only thing on it that is unambiguous.
            #
            # What the gate was protecting against is a mis-placed start, and the
            # ceiling below covers that instead -- see pickup_3_max_travel_cm.
            #
            # A MINIMUM distance was added on top of that on 2026-09-15, and it is
            # a different thing from the withdrawn gate above.  Measured on
            # backoff_20260915.jsonl, after the reverse out of the second area was
            # shortened from 103.78 cm to 30 cm: this leg ended on a wall contact
            # at **179.83 cm** with the car still tracking the line (lateral 0 to
            # 2.29 cm the whole way) -- i.e. it hit something 147 cm short of the
            # build area, and the operator confirmed it stopped halfway.  The
            # arrival test cannot tell that collision from the real one, because
            # both are just "the odometer stopped".
            #
            # So contact now only counts once the car has actually covered the leg.
            # Below the minimum the car keeps driving and the bump is treated as
            # something to get past.  The figure comes from the run that DID reach
            # the build area: 326.78 cm on stable2, measured into the real wall.
            if wall_contact and travel_cm is None:
                # Contact, but no odometer to check it against.  Neither answer is
                # available: it is not an arrival (unknown must not read as
                # satisfied -- the same rule every gate on this route follows) and
                # driving on would push into whatever it is with no way to ever
                # satisfy the minimum.  Hold.
                return RouteIntent("stop", self.state)
            if wall_contact and travel_cm >= self.config.pickup_3_min_travel_cm:
                self._enter(RouteState.BUILD_AREA, now)
            elif (travel_cm is not None
                    and travel_cm >= self.config.pickup_3_max_travel_cm):
                # Runaway net, not an arrival test.  At ~2.6 m this can only fire
                # when the car is not on the line at all: wall2_20260915.jsonl
                # drove 468 cm that way, with encoder_delta reading 40-200 per
                # tick, because the wheels were SPINNING against the wall rather
                # than jamming -- and a contact test can only see "the odometer
                # stopped".  Hold; do not declare an arrival there is no evidence
                # for.
                return RouteIntent("stop", self.state)
            else:
                # Deliberately an else, not a fall-through: after _enter(FINISHED)
                # this must reach the tail of step(), which returns "stop" for a
                # state that neither follows a line nor is watched.  A bare
                # return here would command a V for FINISHED.
                return RouteIntent("v", self.state, speed=self.config.pickup_speed)
        ...
        if self.state is RouteState.BUILD_AREA:
            ...
            if visual.build_block_visible:
                return RouteIntent("strafe", self.state, speed=-abs(self.config.pickup_speed))
        ...
        if self.state is RouteState.BUILD_ACTION:
            if visual.pickup_kind == "fault":
                ...
                # Always back to J3 for another round, including a round that
                # carried only purple.
