...
class ProfiledBlockDetector:
    ...
    def detect(self, frame, *, timestamp_ns: int = 0, frame_index: int = 0) -> BlockDetectionResult:
        ...
        if self.profile.ycrcb_bands:
            ycrcb = self._cv2.cvtColor(frame, self._cv2.COLOR_BGR2YCrCb)
            channel_masks["ycrcb"] = self._band_mask(ycrcb, self.profile.ycrcb_bands)
            mask = self._cv2.bitwise_or(mask, channel_masks["ycrcb"])
        centroid_mask = None
