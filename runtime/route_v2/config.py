...
class RouteV2Config:
    ...
    # How long the route may leave the chassis link COMPLETELY silent.
    #
    # Operator, 2026-09-24: 「不断发送最小的命令给底盘 保活」.  The JDY-31 tears
    # down an idle SPP session after ~13-20 s.  Nothing in the route notices; what
    # breaks is whoever is holding the tty -- the session behind it is gone, so
    # every later read/write fails with `[Errno 5] Input/output error` and the run
    # exits FAULT_SAFE with a V or D still in force on the chassis.
    #
    # The route's own query is rate-limited to 0.15 s and deliberately SKIPS the
    # `d` intents (`_last_intent_kind != "d"` in _dispatch), on the theory that a
    # query arriving mid-move "could disturb it".  That leaves the D-wait windows
    # -- every turn, and blind moves like DIRECT_ORANGE_D330 -- silent for as long
    # as the move lasts, and that is exactly where the link dies.
    #
    # Measured 2026-09-24 (run 20260924_194918): DIRECT_ORANGE_D330 issued
    # `D 330 0 0 80` once and then sent NOTHING for 201 of its 204 ticks.  Its
    # last command went out at 19:51:22; the maintainer logged `link dropped` at
    # 19:51:37 -- a 14.3 s silent window, inside the 13-20 s teardown window --
    # and the run exited `FAULT_SAFE: chassis I/O failure`.  The telemetry shows
    # the link was already dead before that: the encoder tuple froze 14 s ahead of
    # the drop, with encoder_delta / encoder_delta_forward / actual_speed stuck at
    # their last-received values.  Run 20260923_235537 died the same way.
    #
    # So during a D-wait the route now sends its cheapest read-only command, SPD
    # -- `config/runtime.yaml` records it as "commands no motion", and the console
    # already ticks it at exactly this interval for exactly this reason
    # (`chassis.keepalive_s: 5`).  5 s sits well inside the teardown window and is
    # the interval already proven on this link.
    #
    # NOT yet measured: whether a query arriving mid-D disturbs the move.  The
    # 0.15 s exclusion above was written from that fear, not from a measurement --
    # the same kind of unmeasured assumption as "the wheels stall, the skid never
    # happens", which the 2026-09-24 build-area run disproved.  This keepalive is
    # deliberately far slower than 0.15 s for that reason.  Before trusting it on
    # a timed leg, run a turn on the bench (wheels off the ground) with the
    ...
    junction_2_seek_line_vy: int = 50      # after the tag-2 right turn: line is LEFT
    junction_3_seek_line_vy: int = -50     # after the J2 left turn:     line is RIGHT
    pickup_seek_line_vy: int = 50          # after the J3 left turn:     line is LEFT
    ...
    # 左右40 左右60 左右80 一定能找到线了」, then REVISED the same evening after
    # the first field run of that ladder: 「先左右10 再左右20 最后左右30
    # 速度提高一倍」.
    #
    # The ladder was tightened because the field run (20260924_203340) showed the
    # wide version was mostly wasted travel: PICKUP_2_RETURN_TO_LINE confirmed the
    # line 3.2 s into its THIRD leg, i.e. it needed the +20 and -20 rungs and a
    # little of the way to +40, and the 20 s + 13.6 s those legs took match
    # ~2.9 cm/s -- so the car spent 34 s swinging before the line came back under
    # the bar.  A tighter ladder covered at double the speed reaches the same
    # ground in a fraction of the time.
    #
    # Amplitudes, not segment lengths: (10, 20, 30) drives the car to +10, then
    # -10, then +20, -20, +30, -30 -- so each round reaches 10 cm further a side
    # and the move lengths come out 10/20/30/40/50/60.  Positive is LEFT, so the
    # first move is left.  The line is confirmed by seek_line_confirm_frames on
    # every tick, whichever leg is running.
    #
    # The one thing this trades away: the old +-80 ladder could recover a car left
    # 80 cm off the line; +-30 cannot, and a car further out than that will FAULT
    # at the end of the swing rather than hunt further.  The operator chose the
    # tighter ladder knowing the doubled speed, not the reach, is what makes it
    # work -- do not "fix" a fault out here by widening it without asking.
    #
    # This replaces the old single both-ways sweep at seek_line_max_cm (35 cm):
    # +35, then -35, then FAULT.  A block taken from one side can leave the car
    # further off the line than 35 cm, and that search then faulted instead of
    # finding it (run 20260924_195949 faulted in PICKUP_2_RETURN_TO_LINE).
    #
    # The purple area does not use this: its direction is known from which block
    # the arm took, so it reacquires one way (see one_way_direction).
    pickup_return_line_swing_cm: tuple[float, ...] = (10.0, 20.0, 30.0)
    ...
    # Trip 3, the build area.  Arrival is the odometer: the leg ends after
    # pickup_3_arrived_distance_cm of line following, and the build action runs
    # from wherever that puts the car.  Operator, 2026-09-24: 「倒车30cm后
    # 右旋180 巡线D330cm 然后执行搭建动作」.
    #
    # Arrival-on-wall-contact is superseded, and it was measured blind on
    # 2026-09-24 (run 20260924_192550): against this wall the wheels SKID rather
    # than jam, and the encoder counts wheel rotation rather than ground speed,
    # so the odometer kept advancing while the car was pinned.  wall_contact did
    # not fire once in 3188 ticks; the leg ran 1311 ticks (65 s) and 2110 cm of
    # odometer with V 80 0 0 being commanded into the wall throughout, and it
    # ended only when the run was killed by hand.
    #
    # 330 is the operator's number, not a tape reading of the wall.  The wall
    # itself measured 370.06 cm (run 20260923_214138, operator-confirmed), so 330
    # stops the car about 40 cm short of it.  This is NOT the distance gate that
    # was withdrawn on 2026-09-15 -- that one DECLARED an arrival at 62.41 cm
    # from a bad tape measurement and parked the car roughly two metres short of
    # the build area.
    #
    # DELETED 2026-09-23: `pickup_3_max_travel_cm`, the runaway net that held the
    # leg at a ceiling (400 -> 500 on 09-15, 500 -> 360 on 09-16).  It is gone
    # because it could not coexist with the arrival test the leg had then:
    #
    #   * the wall is at **370.06 cm**, PAST the 360 ceiling, so the net fired
    #     inside the normal arrival range -- 0.6 s before the car had even
    #     finished stopping.
    #   * firing was FATAL, not merely early.  The branch returned "stop", so the
    #     next tick was commanded=False, and EncoderContactDetector deliberately
    #     treats a self-stopped car as "not a wall" (it clears stationary_since).
    #     The net therefore switched off the one signal the arrival test read:
    #     measured 3215 ticks (161 s) of a frozen odometer -- encoder_delta 0.0,
    #     one distinct raw ENC tuple -- with wall_contact unreachable for good.
    #
    ...
    pickup_2_seek_line_vy: int = 50        # trip 2, after the right turn: line is LEFT
    ...
    #
    # The leg's arrival is the odometer gate above.  This timeout exists only for
    # the case the gate cannot cover -- ENC stops replying, so travel_cm never
    # advances -- and it must therefore be far ABOVE any realistic traversal, or
    # it stops racing the gate and starts cutting real legs short.
    #
    # 20.0 was the blind-D value and was too tight for that job: at the dry run's
    # 15 cm/s travel model, 330 cm takes 22 s, so the timeout fired 30 cm short of
    # the gate and the leg could not be verified at all (`--dry-run --only
    # DIRECT_ORANGE_D330` reported STOPPED, while the identical build-area leg
    # completed).  On hardware the leg takes ~4 s at pickup_speed, so the real
    ...
    # capped there.
    #
    # Operator, 2026-09-24: 「按照记忆 给策略加一个封顶数量 ... 有一个 就跳过第一个
    # 橙色 直到第二个橙色 ... 下一次回来搭建应该跳过两个」, and on what the car does
    # with a purple on board: 「你应该用视觉找到方块正中 然后执行动作封顶」.
    #
    # Why it exists at all: BUILD_AREA used to act exactly where the 330 cm leg left
    # the car.  With finished buildings already standing there that is the wrong
    # place, and the route cannot see the difference -- BUILD_OCCUPANCY sees ORANGE
    # (run_route_v2.py builds that detector with color=BlockColor.ORANGE), and a
    # capped building is two orange layers with the cap on top, so it looks like a
    # stack still waiting to be capped.  The count is therefore the route's memory,
    # not a vision reading.  Do not widen the colour bands without asking.
    #
    # A "blob passed" is a blob that WAS in view having left it, confirmed over this
    # many frames -- detector flicker must not count as a building.
    build_slide_clear_frames: int = 3
    # 140 cm is the operator's ceiling on the whole slide.  It is a STOP, not a
    # give-up-and-build: reaching it means the detector never let the car past,
    # which is a fault to look at on the field, not a place to drop two blocks.
    #
    # It is generous on purpose -- the measured lateral rate is ~3-4 cm/s at speed
    ...
    #
    # Operator, 2026-09-24: 「你应该用视觉找到方块正中 然后执行动作封顶」.  The
    # build profile's own capture_window is 0.25..0.75 -- half the frame -- which
    # is a "there is a blob here" test, not a "we are centred on it" one, so the
    # centring needs its own, much tighter number.
    build_align_tolerance: float = 0.03
...
def load_route_v2_config(
    ...
    if cfg.pickup_return_line_swing_cm:
        ...
        if list(cfg.pickup_return_line_swing_cm) != sorted(cfg.pickup_return_line_swing_cm):
            # Growing is the whole point of the swing: an out-of-order list would
            # make a later round cover ground an earlier one already searched.
            raise ValueError("pickup_return_line_swing_cm amplitudes must increase")
    ...
    if cfg.direct_orange_distance_cm <= 0 or cfg.orange_direct_timeout_s <= 0:
        raise ValueError("direct orange route settings must be positive")
    if cfg.pickup_3_arrived_distance_cm <= 0:
        raise ValueError("pickup_3_arrived_distance_cm must be positive")
    if cfg.build_slide_clear_frames < 1:
